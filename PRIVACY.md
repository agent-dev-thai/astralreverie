...
- initializes Consent Mode v2 with analytics storage granted for normal reporting while advertising storage, advertising user data, and personalization remain denied;
...
When enabled, GA4 may write standard first-party analytics cookies such as `_ga` and receive standard device/network information. This repository does not include an opt-in consent banner. Each deployer is responsible for an appropriate privacy notice, consent mechanism, retention policy, and regional configuration for their audience. This repository does not claim that enabling GA4 alone is legally compliant in every jurisdiction.
