...
test("renders absolute social metadata from the deployment origin", async () => {
  ...
  assert.match(sharedHtml, /property="og:image" content="https:\/\/gacha\.example\/og\/pull-v3\.jpg\?pull=v1\.seren\.rock"/);
  ...
  const dynamicPreview = await fetch(`${baseUrl}/og/pull-v3.jpg?pull=v1.seren.rock`);
  ...
  const unchangedDynamicPreview = await fetch(`${baseUrl}/og/pull-v3.jpg?pull=v1.seren.rock`, {
    headers: { "If-None-Match": dynamicEtag },
  });
  assert.equal(unchangedDynamicPreview.status, 304);

  const missingPreview = await fetch(`${baseUrl}/og/pull-v3.jpg?pull=v1.unknown`);
